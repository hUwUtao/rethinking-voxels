    ...
    if (validData) {
        ...
        for (; thisLightIndex < MAX_LIGHT_COUNT; thisLightIndex++) {
            ...
            if ((extraData[thisLightIndex] & (1 << 30)) != 0) {
                ...
                if (dist <= maxRange * 1.05) {
                    // Compute type-specific attenuation
                    float atten = 0.0;
                    ...
                    if (ndotl >= 0.001) {
                        ...
                        if (atten >= 0.0001) {
                            ...
                            if (traceResult.w >= 0.01) {
                                #ifdef TRANSLUCENT_LIGHT_TINT
                                    vec3 glassTint = traceResult.xyz; // coneTrace.xyz carries glass color
                                #else
                                    vec3 glassTint = vec3(1.0);
                                #endif
                                vec3 slContrib = lightColor * intensity * atten * traceResult.w * glassTint * ndotl;
                                if (!any(isnan(slContrib))) {
                                    writeColor += slContrib;
                                    atomicMax(lightCoords[thisLightIndex].w, int(10000.5 * length(slContrib)));
                                }
                            }
                        }
                    }
